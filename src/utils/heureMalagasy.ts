...
function formatHeureToMalagasy(date: Date) {
	const hour = date.getHours();
	const minute = date.getMinutes();
	const second = date.getSeconds();
	const state = getTime(hour);

	let new_hour = hour % 12;

	const resultat = `
    ${mlgMap.get(hour)} ora sy
    ${mlgMap.get(minute)} mnitra ${state}    
  `;
	return resultat;
}

const madagascarTime = new Date(new Date().getTime() + 3 * 60 * 60 * 1000);



export { formatHeureToMalagasy };