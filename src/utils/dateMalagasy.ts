const malagasyNumbersMap = new Map([
	[1, "iraika"],
	[2, "roa"],
	[3, "telo"],
	[4, "efatra"],
...
const malagasyMonthsMap = new Map([
	["1", "Janoary"],
	["2", "Febroary"],
	["3", "Martsa"],
	["4", "Aprily"],
...
function getYearToMalagasy(year: string): string {
	let year_to_str = "";
	const last_two = Number(year.substring(2)); // convert to number
	const first_two = Number(year.substring(0, 2));

	year_to_str += malagasyNumbersMap.get(last_two);

	if (first_two === 19) {
		year_to_str += " sy sivan-jato sy arivo";
	} else if (first_two === 20) {
		year_to_str += " sy roa arivo";
	} else if (first_two === 18) {
		year_to_str += " sy valon-jato sy arivo";
	}

	return year_to_str;
}

function formatDateToMalagasy(date: Date) {
	let result: string = "";
	...
	// Day
	result += `${malagasyNumbersMap.get(day)} `;

	// Month
	result += `${malagasyMonthsMap.get(month)} `;

	// Year
	result += getYearToMalagasy(year);
...
function formatDateToFrench(date: string) {
	...
	const french_date_format = `${day}/${month}/${year}`;
	return french_date_format;
}

function getDateYYYMMDD(date: Date): string {
